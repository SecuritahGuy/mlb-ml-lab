"""Ensemble DCN (Deep Cross Network) sequence model + tuned XGB.

Usage:
    poetry run python pipeline/dcn_ensemble.py
"""
...
from mlb_ml_lab.models.sequence import (
    ...
    predict_dcn_multi_task_model,
    save_dcn_model,
    train_dcn_multi_task_model,
)
...
SEED = 42
ENSEMBLE_DIR = "data/models/dcn_ensemble"
...
def main() -> None:
    ...
    print(f"  {len(raw_logs)} game logs, {len(feature_matrix)} feature rows, {len(targets_list)} targets")
    ...
    for target_col in ("target_0.5", "target_1.5"):
        print(f"\n{'=' * 60}")
        print(f"DCN walk-forward — {target_col}")
        print(f"{'=' * 60}")
        ...
        for fold_idx in range(len(TRAIN_SEASONS) - 1):
            ...
            # DCN (multi-task — use single target for both heads)
            dcn_model, _ = train_dcn_multi_task_model(
                Xs_tr, Xc_tr,
                y_tr, y_tr,
                **_DCN_HP,
                seed=SEED,
            ...
            dcn_te_proba_05, dcn_te_proba_15 = predict_dcn_multi_task_model(
                dcn_model, Xs_te, Xc_te,
            )
            dcn_proba = dcn_te_proba_05 if target_col == "target_0.5" else dcn_te_proba_15
            dcn_pred = (dcn_proba > 0.5).astype(np.int32)
            dcn_metrics = classification_metrics(
                y_te.tolist(), dcn_pred.tolist(), dcn_proba.tolist(),
            )

            # Build prediction maps for ensemble alignment
            dcn_proba_map: dict[tuple[int, int], float] = {}
            for i, row in enumerate(test_merged):
                if i < len(dcn_proba):
                    dcn_proba_map[(row["player_id"], row["game_pk"])] = float(dcn_proba[i])
            ...
            for pid, gpk in _keys_te:
                hp = dcn_proba_map.get((pid, gpk), 0.5)
            ...
            if len(ens_y) > 0:
                ...
                ens_metrics = classification_metrics(
                    ens_y.tolist(), ens_pred_bin.tolist(), ens_preds.tolist(),
                )
            else:
                ens_metrics = {"auc": 0.5, "accuracy": 0.5}

            print(f"    XGB       AUC: {xgb_metrics['auc']:.4f}  Acc: {xgb_metrics['accuracy']:.4f}")
            print(f"    DCN       AUC: {dcn_metrics['auc']:.4f}  Acc: {dcn_metrics['accuracy']:.4f}")
            print(f"    Ensemble  AUC: {ens_metrics['auc']:.4f}  Acc: {ens_metrics['accuracy']:.4f}")

            summaries.append({"fold": fold_idx + 1,
                              "xgb_auc": xgb_metrics['auc'],
                              "dcn_auc": dcn_metrics['auc'],
                              "ens_auc": ens_metrics['auc']})

        print(f"\n  === {target_col} Summary ===")
        for s in summaries:
            print(f"    Fold {s['fold']}: XGB={s['xgb_auc']:.4f}  "
                  f"DCN={s['dcn_auc']:.4f}  Ensemble={s['ens_auc']:.4f}")
        if summaries:
            avg_xgb = np.mean([s["xgb_auc"] for s in summaries])
            avg_dcn = np.mean([s["dcn_auc"] for s in summaries])
            avg_ens = np.mean([s["ens_auc"] for s in summaries])
            print(f"    Avg XGB:      {avg_xgb:.4f}")
            print(f"    Avg DCN:      {avg_dcn:.4f}")
            print(f"    Avg Ensemble: {avg_ens:.4f}")

    # ── Final training on all data ───────────────────────────────
    print(f"\n{'=' * 60}")
    print("Training final DCN on all data")
    print(f"{'=' * 60}")
    ...
    print(f"  DCN train: {len(Xs_all)} sequences")

    dcn_final, dcn_meta = train_dcn_multi_task_model(
        Xs_all, Xc_all,
        y_all_seq, y_all_seq,
        **_DCN_HP,
        seed=SEED,
    ...
    print(f"  XGB rows: {len(x_all)}, DCN seqs: {len(Xs_all)}")
    print("\nDone.")
