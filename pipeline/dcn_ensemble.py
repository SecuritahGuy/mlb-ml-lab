...
def main() -> None:
    ...
    for target_col in ("target_0.5", "target_1.5"):
        ...
        for fold_idx in range(len(TRAIN_SEASONS) - 1):
            ...
            dcn_proba = (
                dcn_te_proba_05 if target_col == "target_0.5" else dcn_te_proba_15
            )
            ...
            for i, row in enumerate(test_merged):
                if i < len(dcn_proba):
                    dcn_proba_map[(row["player_id"], row["game_pk"])] = float(
                        dcn_proba[i]
                    )
            ...
            print(
                f"    DCN       AUC: {dcn_metrics['auc']:.4f}  "
                f"Acc: {dcn_metrics['accuracy']:.4f}"
            )
            print(
                f"    Ensemble  AUC: {ens_metrics['auc']:.4f}  "
    ...
    save_dcn_model(
        dcn_final, os.path.join(ENSEMBLE_DIR, "dcn"), sm_f, ss_f, fm_f, fs_f, dcn_meta
    )
