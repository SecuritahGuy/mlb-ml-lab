"""Ensemble gated-hybrid sequence model + tuned XGB on walk-forward splits.

Same as ensemble.py but with learned feature gates on the context path.

Usage:
    poetry run python pipeline/gated_ensemble.py
"""
...
def main() -> None:
    ...
    for target_col in ("target_0.5", "target_1.5"):
        print(f"\n{'=' * 60}")
        print(f"Gated ensemble walk-forward — {target_col}")
        print(f"{'=' * 60}")
        ...
        if all_results:
            ...
            print(f"    Avg Gated:    {avg_hybrid:.4f}")
            print(f"    Avg Ensemble: {avg_ens:.4f}")

    # ── Train final on all data ──────────────────────────
    print(f"\n{'=' * 60}")
    print("Training final gated ensemble on all data")
    print(f"{'=' * 60}")
    ...
    config = {
        "arch": "GatedEnsembleXgbHybrid",
        "models": ["xgb", "gated_hybrid"],
        "weight": 0.5,
        "gating": True,
    ...
    print(f"  XGB rows: {len(x_all)}, Gated seqs: {len(Xs)}")
    print("\nDone.")
