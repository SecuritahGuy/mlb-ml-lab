...
def _merge_rows(feat_rows, tgt_rows, target_col):
    merged = []
    for fr_val, tr_val in zip(feat_rows, tgt_rows):
        row = dict(fr_val)
        row[target_col] = tr_val[target_col]
        merged.append(row)
    return merged
...
def _extract_xgb(merged_rows, target_col, cols=None, imputer=None):
    ...
    for i_val, r in enumerate(merged_rows):
        for j, c in enumerate(cols):
            v = r.get(c)
            x[i_val, j] = float(v) if v is not None else float("nan")
    if imputer is None:
...
test_logs = [
    lg for lg, t in zip(aligned_logs, aligned_tgts) if int(t["date"][:4]) == TEST_SEASON
]
...
train_feat = [
    f for f, t in zip(aligned_feats, aligned_tgts) if int(t["date"][:4]) <= TRAIN_CUTOFF
]
test_feat = [
    f for f, t in zip(aligned_feats, aligned_tgts) if int(t["date"][:4]) == TEST_SEASON
]
...
x_test, y_te_xgb, _, _ = _extract_xgb(
    test_merged, "target_0.5", cols=_xgb_cols, imputer=xgb_imputer
)
...
xgb_metrics = classification_metrics(
    y_te_xgb.tolist(),
    (xgb_te_proba > 0.5).astype(int).tolist(),
    xgb_te_proba.tolist(),
)
...
xgb_proba_map = {
    (r["player_id"], r["game_pk"]): float(xgb_te_proba[i])
    for i, r in enumerate(test_merged)
}
...
for hidden_dim, n_layers in [(64, 2), (128, 2), (256, 2), (128, 3)]:
    ...
    print(
        f"    Hybrid AUC={hybrid_auc:.4f}  Ensemble AUC={ens_auc:.4f}  "
