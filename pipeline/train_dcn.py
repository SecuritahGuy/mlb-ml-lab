"""DCN multi-task hybrid model (Deep & Cross Network + GRU).

Usage:
    poetry run python pipeline/train_dcn.py
"""
...
def main() -> None:
    ...
    # ── Walk-forward ─────────────────────────────────────────────
    print(f"\n{'=' * 60}")
    print("DCN multi-task walk-forward")
    print(f"{'=' * 60}")
    ...
    for fold_idx in range(len(TRAIN_SEASONS) - 1):
        ...
        model, _ = train_dcn_multi_task_model(
            Xs_tr, Xc_tr, y05_tr, y15_tr,
            hidden_dim=64, n_layers=2, dropout=0.2,
            cross_dim=32, num_cross_layers=2,
            learning_rate=3e-4, epochs=120, batch_size=512,
            verbose=True,
        )

        p05, p15 = predict_dcn_multi_task_model(model, Xs_te, Xc_te)
    ...
    # ── Train final model ────────────────────────────────────────
    print(f"\n{'=' * 60}")
    print("Training final DCN multi-task model on all seasons")
    print(f"{'=' * 60}")
    ...
    model, metadata = train_dcn_multi_task_model(
        Xs, Xc, y05, y15,
        hidden_dim=64, n_layers=2, dropout=0.2,
        cross_dim=32, num_cross_layers=2,
        learning_rate=3e-4, epochs=150, batch_size=512,
        verbose=True,
    )
    ...
    p05, p15 = predict_dcn_multi_task_model(model, Xs, Xc)
    m05 = classification_metrics(
