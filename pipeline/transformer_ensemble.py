...
def main() -> None:
    ...
    for target_col in ("target_0.5", "target_1.5"):
        ...
        for fold_idx in range(len(TRAIN_SEASONS) - 1):
            ...
            tf_model, _ = train_transformer_multi_task_model(
                Xs_tr, Xc_tr,
                y_tr, y_tr,  # same target for both heads for single-target mode
                **_TRANSFORMER_HP,
                seed=SEED,
                verbose=False,
            )
            tf_te_proba_05, tf_te_proba_15 = predict_transformer_multi_task_model(
                tf_model, Xs_te, Xc_te,
            )
            ...
            tf_metrics = classification_metrics(
                y_te.tolist(), tf_pred.tolist(), tf_proba.tolist(),
            )
            ...
            for i, row in enumerate(test_merged):
                if i < len(tf_proba):
                    tf_proba_map[(row["player_id"], row["game_pk"])] = float(tf_proba[i])
            ...
            print(f"    Transform AUC: {tf_metrics["auc"]:.4f}  Acc: {tf_metrics["accuracy"]:.4f}")
    ...
    save_transformer_model(tf_final, os.path.join(ENSEMBLE_DIR, "transformer"),
                           sm_f, ss_f, fm_f, fs_f, tf_meta)
