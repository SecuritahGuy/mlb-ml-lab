...
def main() -> None:
    ...
    for fold_idx in range(len(TRAIN_SEASONS) - 1):
        ...
        m05 = classification_metrics(
            y05_te.tolist(), (bag_p05 > 0.5).astype(np.int32).tolist(), bag_p05.tolist(),
        )
        m15 = classification_metrics(
            y15_te.tolist(), (bag_p15 > 0.5).astype(np.int32).tolist(), bag_p15.tolist(),
        )
        ...
        print(f"    Bagged target_0.5 AUC: {row['auc_05']:.4f}  Acc: {row['acc_05']:.4f}")
        print(f"    Bagged target_1.5 AUC: {row['auc_15']:.4f}  Acc: {row['acc_15']:.4f}")
    ...
    if all_results:
        ...
        print(f"    Gain over single multi-task (0.702/0.679): "
    ...
    for seed in BAGGING_SEEDS:
        m, _, _, _, _, _ = load_multi_task_model(os.path.join(MODEL_DIR, f"seed_{seed}"))
        p05, p15 = predict_multi_task_model(m, Xs, Xc)
        bag_p05 += p05 / len(BAGGING_SEEDS)
        bag_p15 += p15 / len(BAGGING_SEEDS)

    m05 = classification_metrics(
        y05.tolist(), (bag_p05 > 0.5).astype(np.int32).tolist(), bag_p05.tolist(),
    )
    m15 = classification_metrics(
        y15.tolist(), (bag_p15 > 0.5).astype(np.int32).tolist(), bag_p15.tolist(),
    )
