...
class StatcastAdvancedFeatures(FeatureExtractor):
    ...
    def features(self) -> list[FeatureMeta]:
        return [
            ...
            FeatureMeta(name="ba", description="Actual batting average", source="statcast"),
            FeatureMeta(name="slg", description="Actual slugging percentage", source="statcast"),
            ...
            # Luck differentials
            FeatureMeta(name="xba_diff", description="xBA minus actual BA", source="statcast"),
            FeatureMeta(name="xslg_diff", description="xSLG minus actual SLG", source="statcast"),
            FeatureMeta(name="xwoba_diff", description="xwOBA minus actual wOBA", source="statcast"),
            ...
            FeatureMeta(name="avg_distance", description="Average batted ball distance (ft)",
                        source="statcast"),
            FeatureMeta(name="max_distance", description="Max batted ball distance (ft)",
                        source="statcast"),
            FeatureMeta(name="avg_hr_distance", description="Average home run distance (ft)",
                        source="statcast"),
            FeatureMeta(name="ev95plus", description="Count of 95+ mph exit velocity",
                        source="statcast"),
            FeatureMeta(name="barrels", description="Count of barrels", source="statcast"),
        ]
        ...
        **kwargs: Any,
        ...
        for log in game_logs:
            ...
            rows.append({
                ...
                "xslg": xslg,
                # Luck differentials
                "xba_diff": (xba - ba) if (xba is not None and ba is not None) else None,
                "xslg_diff": (xslg - slg) if (xslg is not None and slg is not None) else None,
                "xwoba_diff": (xwoba - woba) if (xwoba is not None and woba is not None) else None,
