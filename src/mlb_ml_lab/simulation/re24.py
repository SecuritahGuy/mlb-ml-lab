...
def _parse_bases_before(runners: list[dict[str, Any]]) -> tuple[int, int, int]:
    bases = [0, 0, 0]
...
def _parse_bases_after(runners: list[dict[str, Any]]) -> tuple[int, int, int]:
    bases = [0, 0, 0]
    for runner in runners:
...
def compute_re24(plays: list[dict[str, Any]]) -> dict[tuple[int, int, int, int], float]:
    """Compute run expectancy for all 24 base-out states.

    Tracks runs remaining from each state to the end of its inning,
    which is the canonical RE24 definition.

    Returns:
        Mapping from ``(1B, 2B, 3B, outs)`` → expected runs to end of inning.
    """
    state_to_runs: dict[tuple[int, int, int, int], list[float]] = defaultdict(list)
    ...
    for play in plays:
        ...
        if outs_after >= 3:
            for state, runs_at_state in states_in_inning:
                ...
                state_to_runs[state].append(remaining)
            states_in_inning = []
            current_state = (0, 0, 0, 0)
            cumulative_runs = 0.0

    re24 = {}
    for state, runs_list in state_to_runs.items():
...
def re24_to_array(re24: dict[tuple[int, int, int, int], float]) -> np.ndarray:
    arr = np.zeros((8, 3))
