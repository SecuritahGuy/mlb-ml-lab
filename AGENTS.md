...
- Python 3.13, Poetry, `src/mibl/` package layout
...
- `src/mibl/models/` for models; `src/mibl/features/` for feature engineering; `experiments/` for notebooks.
- Walk-forward validation (never random train/test split) — sports data is temporally dependent.
- Feature engineering lives in `src/mibl/features/` (part of the installable package). The `pipeline/` directory at project root is for model training/prediction only.
